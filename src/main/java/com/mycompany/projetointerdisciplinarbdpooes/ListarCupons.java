 ...
 * @author PC-02
 */
public class ListarCupons extends JFrame {
    ...
    public ListarCupons(int idUsuario) throws SQLException {
        super("Cupons");
        criarJTable();
        criarJanela();
    ...
    private class btInserirListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent ae) {
            InserirCupom ic = new InserirCupom(modelo);
            ic.setVisible(true);
        }
    }
    ...
    private class btAlterarListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if (linhaselecionada >= 0) {
                int idCupom = (int) tabela.getValueAt(linhaselecionada, 0);
                AtualizarCupom ac = new AtualizarCupom(modelo, idCupom, linhaselecionada);
                ac.setVisible(true);
...
//                System.out.println("Id selecionado: " + idcontato);
        ...
        public void actionPerformed(ActionEvent ae) {
            ...
            if (linhaselecionada >= 0) {
                ...
                if (dialogResult == 0) {
                    ...
                    Resposta resposta = dao.remover(idcupom);
    ...
    public static void pesquisar(DefaultTableModel model) {
        model.setNumRows(0);
        CupomDAO dao = new CupomDAO();
        for (Cupom c : dao.getCupons()) {
            DecimalFormat decimalFormat = new DecimalFormat("#0.00");
            String valorDoDesconto = " ";
            if (c.getTipoDesconto().equals("p")) {
                 valorDoDesconto =  decimalFormat.format(c.getValorDesconto())+"%";
            } else {
                valorDoDesconto =  "R$ " + decimalFormat.format(c.getValorDesconto());
            }

            model.addRow(new Object[]{c.getId(), c.getCodigoCupom(), valorDoDesconto, c.getLegendaDesconto()});
        }
    }
